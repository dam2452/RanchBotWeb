...
class AdjustedVideoService:
    ...
    async def get_cached(self, clip_index: int, left_adjust: int, right_adjust: int) -> bytes | None:
        return await self._file_cache.get(self._cache_key(clip_index, left_adjust, right_adjust))
    ...
    async def save_to_cache(
        ...
        await self._file_cache.put(key, data)
