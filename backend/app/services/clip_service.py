...
class ClipService:
    ...
    async def get_clip_thumbnail(
        ...
        await self._thumbnail.update_sidecar(clip_name, content_hash)

        if if_none_match and if_none_match.strip('"') == content_hash:
            return thumbnail_not_modified_response(content_hash)

        await self._thumbnail.cache(content_hash, thumbnail_data)
    ...
    async def save_clip(self, clip_name: str, token: str) -> dict:
        ...
        await self._thumbnail.invalidate(clip_name)
        return {
            "status": "success",
